...
- When Sketch collapses an input Frame to the content bounds of a Group, structural repair may add a detached sibling background only when exported Padding explains the lost bounds within 3px. It restores safe solid fill, stroke, weight, alignment, and corners without converting the Group or moving its children. Auto Layout remains blocked for that collapsed Group.
